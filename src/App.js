...
const VideoStream = () => (
  <div className="video-stream-container">
    ...
    <iframe
      ...
      src="https://www.youtube.com/embed/iBgiSII2WBI?si=QZGO5cVriaodg8zx"
...
function App() {
  ...
  useEffect(() => {
    const fetchHistoricalData = async () => {
      ...
      const historyQuery = query(historyRef, orderByKey(), limitToLast(10));
    ...
    const latestReadingRef = ref(db, 'bwise_current');
    const unsubscribe = onValue(latestReadingRef, (snapshot) => {
      const data = snapshot.val();
      if (data) {
        const processedData = {
          ...data,
        ...
        const newPoint = {
          ...
          y: processedData.weight
        };

        setWeightHistory(prevHistory => {
          if (
            prevHistory.length > 0 &&
            prevHistory[prevHistory.length - 1].x === newPoint.x
          ) {
            return prevHistory;
          }
  ...
  const weightChartOptions = {
    ...
    stroke: { curve: 'smooth' },
    title: { text: 'Hive Weight History', align: 'left' },
    markers: { size: 0 },
    xaxis: { type: 'datetime' },
    yaxis: {
      title: { text: 'Weight (g)' },
        ...
        <main>
          ...
          <div className="grid-container">
            <div className="grid-item">
              <GaugeChart value={sensorData.temperature} title="Temperature" unit="°C" min={0} max={50} colors={[tempColor]} />
            </div>
            <div className="grid-item">
              <GaugeChart value={sensorData.humidity} title="Humidity" unit="%" min={0} max={100} colors={[humidityColor]} />
            </div>
            <div className="grid-item">
              <GaugeChart value={sensorData.sound_voltage_num} title="Sound" unit="dB" min={-100} max={0} colors={[soundColor]} />
            </div>

            <div className="grid-item large"><VideoStream /></div>
